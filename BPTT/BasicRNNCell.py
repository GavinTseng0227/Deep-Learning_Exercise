...
class BasicRNNCell:
    ...
    def fprop(self, data):
        ...
        for i in range(n_samples):
            # Append the input vector for later re-usability
            self.xs.append(data[i:i + 1].T)

            # Computation of linear components
            # list [-1]: the last element in a sequence
            h = np.dot(self.W_hx, self.xs[-1]) + np.dot(self.W_hh, self.hs[-1]) + self.bh
            # ReLU non-linearity: [0, 1]
            h = np.clip(h, 0, None)
            self.relu_idx.append(h == 0)
            # Storing of hidden layer values
            self.hs.append(h)
    ...
    def bprop(self, dys):
        ...
        for dy in reversed(dys):
            ...
            # Bprop into h_raw
            self.dW_hx += np.dot(dh, self.xs[-1].T)
            self.dbh += dh
